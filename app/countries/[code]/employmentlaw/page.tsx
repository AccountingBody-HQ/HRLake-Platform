...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title, description, url: `https://hrlake.com/countries/${code.toLowerCase()}/employmentlaw/`, siteName: 'HRLake', type: 'website' },
  }
}
...
function fmtFrequency(value: string | null | undefined): string {
  if (!value) return '—'
  const map: Record<string, string> = { monthly: 'per month', weekly: 'per week', bi_weekly: 'bi-weekly', per_hour: 'per hour', per_day: 'per day', annual: 'per year' }
...
export default async function EmploymentLawPage({ params }: PageProps) {
  ...
  const ruleCards = [
    { label: 'Minimum Wage',     value: employmentRules?.minimum_wage ? `${fmtCurrency(employmentRules.minimum_wage, country.currency_code)} ${fmtFrequency(employmentRules.payroll_frequency)}` : '—' },
    ...
    { label: 'Sick Leave',       value: fmt(employmentRules?.sick_leave_days, ' days') },
    { label: 'Notice Period',    value: employmentRules?.notice_period_days ? `${employmentRules.notice_period_days} ${employmentRules.notice_period_days === 1 ? 'day' : 'days'}` : '—' },
    { label: 'Probation Period', value: fmt(employmentRules?.probation_period_days, ' days') },
    { label: 'Maternity Leave',  value: fmt(employmentRules?.maternity_leave_weeks, ' weeks') },
    { label: 'Paternity Leave',  value: fmt(employmentRules?.paternity_leave_weeks, ' weeks') },
    { label: 'Overtime Rate',    value: employmentRules?.overtime_rate ? `${employmentRules.overtime_rate}x standard rate` : '—' },
    { label: 'Max Working Hours',value: fmt(employmentRules?.working_hours_max, ' hrs/week') },
    { label: '13th Month Pay',   value: employmentRules ? (employmentRules.thirteenth_month_pay ? 'Required' : 'Not required') : '—' },
  ]

  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: toJsonLd(getBreadcrumbStructuredData([
        { name: 'Home', href: '/' },
        ...
        <section className="relative bg-slate-950 overflow-hidden">
          ...
          <div className="relative max-w-7xl mx-auto px-6 lg:px-8 pt-12 pb-14">
            ...
            <div className="flex flex-wrap items-start justify-between gap-6">
              ...
              <Link href={`/countries/${code.toLowerCase()}/`} className="shrink-0 flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-blue-400/40 text-slate-300 hover:text-white rounded-xl px-5 py-3 text-sm font-medium transition-all">&larr; {country.name} Overview</Link>
            </div>
          </div>
        </section>
            ...
            <div className="grid gap-10 lg:grid-cols-3">
              <div className="lg:col-span-2 space-y-8">
                {!employmentRules && !workingHours && !termination ? (
                  <div className="rounded-2xl border border-amber-200 bg-amber-50 px-6 py-5 text-sm text-amber-800"><strong>Data coming soon.</strong> Employment law data for {country.name} is being prepared.</div>
                ) : (
                  <>
                    {employmentRules && (
                      <div>
                    ...
                    {workingHours && (
                      <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
                        <div className="flex items-center gap-3 mb-6">
                          <div className="w-9 h-9 rounded-xl bg-blue-50 flex items-center justify-center shrink-0"><Clock size={18} className="text-blue-600" /></div>
                          <h2 className="font-serif text-2xl font-bold text-slate-900">Working Hours</h2>
                        </div>
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
                          {workingHours.standard_hours_per_week != null && (<div className="rounded-xl bg-slate-50 border border-slate-100 p-4"><p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">Standard Hours</p><p className="text-xl font-bold text-slate-900">{workingHours.standard_hours_per_week}<span className="text-sm font-medium text-slate-500 ml-1">hrs/week</span></p></div>)}
                          {workingHours.maximum_hours_per_week != null && (<div className="rounded-xl bg-slate-50 border border-slate-100 p-4"><p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">Maximum Hours</p><p className="text-xl font-bold text-slate-900">{workingHours.maximum_hours_per_week}<span className="text-sm font-medium text-slate-500 ml-1">hrs/week</span></p></div>)}
                          {workingHours.overtime_rate_multiplier != null && (<div className="rounded-xl bg-slate-50 border border-slate-100 p-4"><p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">Overtime Rate</p><p className="text-xl font-bold text-slate-900">{workingHours.overtime_rate_multiplier}<span className="text-sm font-medium text-slate-500 ml-1">x rate</span></p></div>)}
                          {workingHours.overtime_threshold_daily != null && (<div className="rounded-xl bg-slate-50 border border-slate-100 p-4"><p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">Daily OT Threshold</p><p className="text-xl font-bold text-slate-900">{workingHours.overtime_threshold_daily}<span className="text-sm font-medium text-slate-500 ml-1">hrs/day</span></p></div>)}
                          {workingHours.overtime_threshold_weekly != null && (<div className="rounded-xl bg-slate-50 border border-slate-100 p-4"><p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">Weekly OT Threshold</p><p className="text-xl font-bold text-slate-900">{workingHours.overtime_threshold_weekly}<span className="text-sm font-medium text-slate-500 ml-1">hrs/week</span></p></div>)}
                        </div>
                        {workingHours.notes && <p className="text-sm text-slate-500 leading-relaxed border-t border-slate-100 pt-4">{workingHours.notes}</p>}
                        {workingHours.source_url && <a href={workingHours.source_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs text-blue-500 hover:text-blue-600 mt-3 transition-colors">Official source &nearr;</a>}
                      </div>
                    )}
                    {termination && (
                      <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
                        <div className="flex items-center gap-3 mb-6">
                          <div className="w-9 h-9 rounded-xl bg-red-50 flex items-center justify-center shrink-0"><UserX size={18} className="text-red-500" /></div>
                          <h2 className="font-serif text-2xl font-bold text-slate-900">Termination &amp; Redundancy</h2>
                        </div>
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
                          {termination.notice_period_min_days != null && (<div className="rounded-xl bg-slate-50 border border-slate-100 p-4"><p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">Min Notice Period</p><p className="text-xl font-bold text-slate-900">{termination.notice_period_min_days}<span className="text-sm font-medium text-slate-500 ml-1">{termination.notice_period_min_days === 1 ? 'day' : 'days'}</span></p></div>)}
                          {termination.probation_period_max_months != null && (<div className="rounded-xl bg-slate-50 border border-slate-100 p-4"><p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">Max Probation</p><p className="text-xl font-bold text-slate-900">{termination.probation_period_max_months}<span className="text-sm font-medium text-slate-500 ml-1">{termination.probation_period_max_months === 1 ? 'month' : 'months'}</span></p></div>)}
                          <div className="rounded-xl bg-slate-50 border border-slate-100 p-4"><p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">Severance</p><p className="text-xl font-bold" style={{ color: termination.severance_mandatory ? '#10b981' : '#64748b' }}>{termination.severance_mandatory ? 'Mandatory' : 'Not required'}</p></div>
                          {termination.garden_leave_allowed != null && (<div className="rounded-xl bg-slate-50 border border-slate-100 p-4"><p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">Garden Leave</p><p className="text-xl font-bold" style={{ color: termination.garden_leave_allowed ? '#10b981' : '#64748b' }}>{termination.garden_leave_allowed ? 'Permitted' : 'Not permitted'}</p></div>)}
                          {termination.unfair_dismissal_threshold_months != null && (<div className="rounded-xl bg-slate-50 border border-slate-100 p-4"><p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">Unfair Dismissal From</p><p className="text-xl font-bold text-slate-900">{termination.unfair_dismissal_threshold_months}<span className="text-sm font-medium text-slate-500 ml-1">months</span></p></div>)}
                        </div>
                        <div className="space-y-4 border-t border-slate-100 pt-5">
                          {termination.notice_period_formula && (<div><p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">Notice Period Formula</p><p className="text-sm text-slate-700 leading-relaxed">{termination.notice_period_formula}</p></div>)}
                          {termination.severance_formula && (<div><p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">Severance Calculation</p><p className="text-sm text-slate-700 leading-relaxed">{termination.severance_formula}</p></div>)}
                          {termination.redundancy_rules && (<div><p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">Redundancy Rules</p><p className="text-sm text-slate-700 leading-relaxed">{termination.redundancy_rules}</p></div>)}
                          {termination.notes && (<div className="rounded-xl bg-amber-50 border border-amber-100 p-4"><p className="text-xs font-bold uppercase tracking-widest text-amber-600 mb-1">Important Notes</p><p className="text-sm text-amber-800 leading-relaxed">{termination.notes}</p></div>)}
                        </div>
                        {termination.source_url && <a href={termination.source_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs text-blue-500 hover:text-blue-600 mt-4 transition-colors">Official source &nearr;</a>}
                      </div>
                    )}
                    {sanityArticle?.body && (
                      <div className="prose max-w-none">
                        <h2 className="font-serif text-2xl font-bold text-slate-900 mb-6">{country.name} Employment Law — Full Guide</h2>
                        <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm"><PortableText value={sanityArticle.body} /></div>
                      </div>
                    )}
                    <div className="rounded-2xl border border-teal-100 bg-teal-50 p-6 flex flex-wrap items-center justify-between gap-4">
                      ...
                      <Link href={`/eor/${code.toLowerCase()}/`} className="shrink-0 inline-flex items-center gap-2 bg-teal-600 hover:bg-teal-500 text-white font-semibold px-5 py-3 rounded-xl text-sm transition-colors">Explore EOR <ArrowRight size={14} /></Link>
                    </div>
              ...
              <div className="space-y-5">
                ...
                <div className="rounded-2xl p-6 overflow-hidden relative" style={{ backgroundColor: '#0d1f3c' }}>
                  ...
                  <div className="relative">
                    ...
                    <Link href={`/eor/${code.toLowerCase()}/`} className="block rounded-xl bg-teal-600 hover:bg-teal-500 px-4 py-3 text-center text-sm font-bold text-white transition-colors">Explore EOR for {country.name}</Link>
                  </div>
                </div>
                <div className="rounded-2xl p-6 overflow-hidden relative" style={{ backgroundColor: '#0d1f3c' }}>
